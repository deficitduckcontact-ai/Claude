<script lang="ts">
  ...
  import { mode as site } from '$lib/firebase.svelte';
...
<div class="auth card">
  ...
  <button class="btn block google" onclick={google} disabled={!site.live} title={site.live ? '' : 'Works on the live site'}>
  ...
  {#if site.resolved && !site.live}<div class="todo-box"><b>Demo mode:</b> accounts are stored in this browser only. On the live site they're real accounts with email verification.</div>{/if}
</div>
